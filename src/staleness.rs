    ...
    T: WithTimestamp + Clone,
{
    pub fn new(config: StalenessConfig) -> Self {
        if config.enable_immediate_expiration {
            ...
            {
                panic!("Immediate staleness expiration requires 'tokio' feature. Either enable the 'tokio' feature or set 'enable_immediate_expiration = false' for lazy checking.");
            }
        }
...
mod tests {
    ...
    fn test_constrained_heap_size_constraint() {
        ...
        assert!(heap
            .try_add("key1", msg1, Duration::from_millis(50))
            .is_ok());
        assert!(heap
            .try_add("key2", msg2, Duration::from_millis(100))
            .is_ok());
